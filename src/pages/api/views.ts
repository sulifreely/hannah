...
import { getViewsMap, incrementViews } from '../../lib/views';
...
/** GET /api/views?slugs=a,b,c&contentType=blog — 批量只读查询，不计数。 */
export const GET: APIRoute = async ({ url }) => {
  ...
  const views = await getViewsMap(contentType, slugs);
  return json(
    { contentType, views },
...
/** POST /api/views { slug, contentType } — 阅读量 +1，返回自增后的最新值。 */
export const POST: APIRoute = async ({ request }) => {
  ...
  const views = await incrementViews(contentType, slug);
  return json({ slug, contentType, views });
};
