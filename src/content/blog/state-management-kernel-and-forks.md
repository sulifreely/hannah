...
description: 整合自己之前写的 Redux、React Redux、Jotai 三篇源码精读，抽出一个贯穿所有状态管理方案的最小内核，沿着组织形态、变化发现机制、UI 耦合深浅三条分歧轴，重新理解 Redux、Jotai、Zustand、MobX/Valtio、Signals、XState 的设计取舍。
tags:
  - 状态管理
  - 前端框架
  - 源码精读
...
前面是分轴讲的，每条轴单独配了一张图、一张表，读到这里容易忘了它们本来同属一个坐标系。这里把内核和三条轴拼在一张架构图里：
...
从这张图能看出，Redux、Zustand、Jotai 这几个名字会在不止一条轴上重复出现。它们在轴一可能站在同一个点（比如 Redux 和 Zustand 都是“树”），却会在轴二分道扬镳。一个方案真正的设计取舍，要看它在三条轴上的位置组合，不能只看其中一条。
...
这张表不是用来选一个赢家的。一个项目里，状态管理方案常常会叠着用：例如 Zustand 存 UI 状态，某个复杂表单单独用 XState 建模。它们各自解决不同问题，谈不上互相替代。
...
Redux 选了树、显式 dispatch、外部接入；Jotai 选了图、读时收集、外部接入；Zustand 证明了外部接入这条路径上，仪式感是可以单独拆掉的；MobX、Valtio 换了一整套写时代理的发现机制；XState 换了个维度，去约束状态转移本身；Signals 则在第三条轴上走到了另一个极端，把响应式嵌进了渲染系统内部。
