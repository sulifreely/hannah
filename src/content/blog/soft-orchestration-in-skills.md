...
description: 聊聊在 Skill 中如何用链式、并行、路由、循环、编排和层级这几种模式组织 Agent 工作流，以及在概率性执行里，如何通过状态、门禁、校验和工具协议保障确定性。
tags:
  - AI
  - Agent
  - Skill
...
提到 Agent 工作流，大家可能会先想到一个很硬的东西：状态机、DAG、workflow engine、可视化节点编排，最好还能拖拖拽拽。


这些当然很重要。但在很多日常 AI Coding 场景里，我们其实并不一定需要一套重型编排系统。更多时候，我们需要的是一种更轻的东西：把任务拆开，把顺序讲清楚，把分叉条件写明白，把什么时候该停下来校验说清楚。


这就是我理解的 **Skill 中的软编排**。


所谓“软”，不是说它随便，也不是靠玄学提示词祈祷模型听话。它更像一份写给 Agent 的工程操作手册：不直接把每一步固化成代码节点，但会把工作流的结构、约束、检查点和退出条件写进 Skill，让模型在执行时沿着相对稳定的轨道前进。
...
但任务一复杂，事情就会开始长出岔路。比如写一篇技术方案，需要先理解背景，再查代码，再比较方案，再写文档，再做验证。这里面有顺序，有分支，有重复，也有不同角色之间的协作。
...
flowchart TD
    ...
    C{步骤之间是什么关系}
    D[链式 Chain - 一步接一步]
    E[并行 Parallel - 同时收集多个结果]
    F[路由 Route - 按条件选择路径]
    G[循环 Loop - 执行检查修正]
    H[编排 Orchestrate - 中心协调多个能力]
    I[层级 Hierarchy - 父任务拆给子任务]
    A --> B
    B --> C
    C --> D
    C --> E
...
这些模式并不是互斥的。真实工作流里，它们经常混在一起。比如一个父 Agent 先路由任务类型，然后并行启动几个子任务，最后进入一个检查循环。


听起来有点像项目管理，只是项目经理变成了一段 Skill。也不能说完全没有道理。
...
链式适合那些天然有先后依赖的任务。比如修 bug 时，通常不能一上来就改代码。更稳的顺序是：先复现，再定位，再修改，再验证。
...
一个很典型的例子是 [`executing-plans`](https://github.com/obra/superpowers/blob/main/skills/executing-plans/SKILL.md) 这个 Skill。它的流程基本就是一条链：先读取计划，再批判性审阅计划，接着逐个执行任务，每个任务都要标记状态、按步骤执行、运行验证，最后再进入收尾 Skill。


这里的重点不是“它有 1、2、3、4”，而是每一步都依赖前一步的产物。没有读完计划，就不能开始执行；验证没过，就不能把任务标成完成；所有任务没完成，就不能进入最终收尾。


所以 [`executing-plans`](https://github.com/obra/superpowers/blob/main/skills/executing-plans/SKILL.md) 本质上是在把“照着计划做事”这件事，变成一条带检查点的执行链。
...
并行的收益是快，但风险是散。Skill 里的重点，就是把“散”收回来：每个分支各查各的，但都按同一种格式把证据交回来。
...
路由最怕的是条件模糊。条件一模糊，模型就会凭语感选路。语感不是不能用，但它不应该成为唯一依据。
...
人写代码也不是一次写完的。更真实的过程是：写一点，跑一下，看错误，再改一点。模型也一样，而且更需要这种外部反馈。
...
在 Skill 里，循环一定要写清楚退出条件。否则它很容易变成“再试一次，再试一次，再试一次”，最后把上下文和耐心一起烧干。
...
这类规则看起来像管理学废话，但对 Agent 很有用。因为它告诉模型：你可以借力，但不能把最终判断外包出去。
...
层级模式的问题是：一旦拆得太深，父任务很容易失去全局视角，子任务也容易在局部最优里越走越远。
...
阶段定义的重点，是让模型知道“现在走到哪里了”。如果没有阶段，模型很容易把探索、决策、执行、总结混在一起。
...
工具协议让工作流从“想做什么”变成“怎么做”。它也是保障确定性的关键部分，因为工具结果比模型猜测更可靠。
...
如果 Agent 什么都问，用户会觉得它像一个刚入职第一天的同事，连“要不要保存文件”都要举手确认；如果 Agent 什么都不问，又会在关键分叉上擅自替用户做决定，最后看起来很勤快，实际把方向跑偏了。
...
答案要稍微绕一点：不能指望模型本身完全确定，但可以通过工程约束，让关键结果尽量确定。
...
软编排的价值，不是替代所有工程系统。它更像给 Agent 配一套操作规程：让它在多数日常任务里少走弯路，也让人更容易理解和接管。
...
到最后，Skill 不是魔法咒语，而是一份认真写过的团队协作规范。


只不过这次，团队成员里多了一个很会写、很会猜、偶尔也会一本正经胡说八道的 Agent。我们要做的，不是期待它突然变成完全确定的程序，而是给它一条足够清楚的路：该自由发挥的地方自由发挥，该踩刹车的地方就踩刹车。
