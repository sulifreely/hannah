...
![左边是重型编排引擎（成堆齿轮与刚性节点），右边是一份轻量操作手册，Agent 照着清单走](/images/blogs/soft-orchestration-in-skills/01-comparison-soft-vs-hard.jpg)
...
链式适合那些天然有先后依赖的任务。比如修 bug 时，通常不能一上来就改代码。更稳的顺序是：先复现，再定位，再修改，再验证。火还没看清楚从哪里冒出来，就急着往墙上浇水，最后多半只会把屋子弄得更湿。
...
并行的收益是快，但风险是散。Skill 里的重点，就是把“散”收回来：每个分支各查各的，但都按同一种格式把证据交回来。像几个人分头去不同书架找资料，最后还是要回到同一张桌子上摊开。
...
答案要稍微绕一点：不能指望模型本身完全确定，但可以通过工程约束，让关键结果尽量确定。我们不是把风变成石头，而是给帆、舵和锚各自安排好位置。


![左侧是散乱的概率输出，经过几道护栏逐层收拢，最终汇聚成一个带对勾的验证印章](/images/blogs/soft-orchestration-in-skills/02-framework-determinism.jpg)
...
如果把这四层排成一条从“可变”到“必须可验证”的光谱，会更直观：
...
越往右，越不能靠模型“自由发挥”，越需要工程手段兜底。下面六条，就是把光谱右侧那几层压稳的具体做法：让可以飘的地方继续飘，让必须落地的地方真的落地。
