---
name: y-sketch
description: 为任意文章（博客、笔记、文档、公众号、分享稿）生成 sketch-meme 风格封面图和正文插图——手绘黑线稿 + 彩色填充 + 按情景挑选的网络梗图角色（悲伤蛙 / 柴犬 / 哲学迅猛龙 / Wojak 等，按情景平权挑选，不默认用 Wojak）。当用户提到封面图、文章插图、meme 风格配图，或写作流程需要产出封面时调用。
---
...
**从整个梗图角色库里，按情景挑最贴切的那个角色**，不预设某一家。先问："这个情绪 / 这个矛盾，哪个梗一出来大家就懂、且自带笑点或反讽？"——是认命就用 Okay Guy，是社畜内耗就用章鱼哥，是阴阳吐槽就用 Doge，是灵魂拷问就用 Philosoraptor，是看穿真相就用 Futurama Fry。谁最精准用谁，**主角 ≤ 3 个**。

选之前先做一次**平权扫描**：从角色库里挑 2-3 个候选，其中至少一个不是 Wojak 家族，逐一自问"这个梗好笑/好讽在哪、贴不贴当前情绪"，再决定用哪个。Wojak 表情谱系全、几乎什么情绪都能凑合套上，这恰恰是它最大的陷阱——因为"总能用"，很容易变成不假思索的默认选项，读者看多了也会审美疲劳。贴切度打平时，优先选专属梗，不选 Wojak。

如果这不是当前任务里的第一张图，回头看一眼前面已经用过哪些角色/家族，这次尽量换一个不同的，除非场景本身要求重复用同一角色（比如四宫格里同一角色的表情逐格变化）。
...
布局（三个位置各自对应一种情绪，按情景挑角色，不要习惯性套 Wojak 三连）：
- 左（低分）：需要"简单/懵懂"的情绪 + 简单观点标签——按情景可选派大星、Plain Wojak、背手负鼠等
- 曲线顶部（中分）：需要"痛苦/崩溃/纠结"的情绪 + 复杂方案 bullet list——按情景可选悲伤蛙 Pepe、SpongeGar、Crying Wojak 等
- 右（高分）：需要"看穿真相后的平静或自信"的情绪 + 和左侧完全相同的简单观点——按情景可选 Futurama Fry、Philosoraptor、Chad Wojak 等

三个位置不必都出自同一角色家族，只要全图统一视觉语言（都手绘线稿），可以分别挑最贴切的表情。
...
**不预设某一家角色，按情景挑最贴切的。** 先明确要传达的情绪或矛盾，再从下表里找那个"一出来大家就懂"的梗。下面故意把专属梗角色排在前面：认命、社畜内耗、阴阳吐槽、灵魂拷问、看穿真相这些情绪，用对应的专属梗几乎总是比套用 Wojak 变体更精准、更好笑，选角色时先看这张表。

Wojak 家族排在后面，不是因为它不好用，恰恰是因为它太好用——表情谱系全，几乎任何情绪都能找到一个凑合的变体。这种"总能用"的特性最容易让人不假思索地默认选它，选多了读者也会审美疲劳。把它放在候选列表最后，是刻意提醒自己：只有专属梗库里确实没有更贴切的选项时，才轮到它。
...
### 专属梗角色（先看这张表，多数情绪都能找到比 Wojak 更贴切的选项）
...
### Wojak 家族（谱系最全，也最容易被过度使用，专属梗表里没有更贴切选项时才用）
...
- 标签足够短，没有变成说明书？
- 角色是不是按情景挑的最贴切那个？有没有真的先看专属梗角色表、比较过至少一个非 Wojak 候选，而不是习惯性用 Wojak？有没有多风格混堆？
- 如果这批任务里不止一张图，角色/家族有没有重复到让人审美疲劳？能不能换一个没用过的家族？
